...
## 5bc. v1929: trin 2 bygget efter bud 3 (Idas klik 14/9 kl. 22.05)

`prForslagKilder/prForslagByg/prForslagHtml/prForslagGem/prAndet` er erstattet af trin 2-flowet: `prT2()` (tilstand i `PR.t2`: skaerm antal·raekke·mg·spm·ide, antal, forslag[{kode,navn,grund,dag,gemt,titel}], eksisterende, aktiv, mg, q, svar[3], valg[3], skriv, titel, format), `prAntalHtml` + hjulet (`.pr-hjul`, scroll-snap 44 px pr. tal, `prHjulInit/prHjulScroll/prHjulTil`, `prAntalGem` skriver MAAL_UGE + kunde_maal.opslag_uge som perfMaalRet), `prRaekkeHtml` (svarkort `prSvarKort`: `.pr-svar`, cirkel `.pr-svar-rd`, dim for det, der ligger), `prStartOpslag(i)`, `prMgHtml` + `prMgFordeling` (brief.maalgruppe de sidste 28 dage; `maalgruppeNavne()` er kilden), `prSpm(kode)` (tre spørgsmål pr. søjle, målgruppens fornavn sat ind, Drejebogens `ideer`-linje som undertekst, 1B får kundens `strategi.kunde.tekst`), `prSpmHtml` (kort fra `prForslagKilder(kode, "situationer"|"eksempler"|"hooks")` = kundens `egneFor` + `BS_SOEJLER` i du-form via `bsTiltale(t,true)`, max 5, + »Skriv selv« med `taleKnapHtml("prSkriv")`), `prSvarVaelg/prSvarNu`, `prIdeHtml` (titel = første sætning af svar 1, dag-chips med dato, format-chips Reel·Kort-reel·Karrusel med søjlens format forvalgt), `prIdeGem` (ideAdd Planlagt; brief.beskrivelse = spørgsmål+svar; brief.maalgruppe kun ved flere), `prT2Naeste`, `planRitTilbage` (topknappen: ét skridt tilbage i trin 2, ellers luk).

- **Målt i selen (390, stub-kunden »Test« uden Drejebog-persona):** antal-hjulet 236 px bredt, 3 forvalgt; Næste → »Start med det, ugen mangler« med 1A·1B·2A (alle »Mangler i uge 39«, dage ti/to/lø) og knappen »Find idéen til 1A«; 1A spørgsmål 1 viser fem husets eksempler som kort + Skriv selv med mikrofon; svar 2 viser »Dit svar før«; »Her er idéen« med titel, »Ti 22.« og »Reel« valgt; Gem → flowet går selv videre til 1B, og rækkefølgen viser »1A · Produkt · Planlagt · titel · tirsdag« (dim). Sele uden base: den røde »ikke gemt centralt« er selens vilkår.
- **Målt LIVE v1929 i Idas kundesession (Acorns, 14/9 kl. 22.40, uden at gemme):** ugemål 4 læses ind i hjulet; rækkefølgen viser kundens egne søjlenavne (1A · Produkt eller ydelse, 1B · Målgruppens behov / udfordring, 2A · Mennesker, 2B · Forretning); 1B spørgsmål 1 hedder »Hvilken situation kender Charlotte fra sin hverdag lige nu?«, underteksten er Charlottes Drejebog-tekst, og de tre kort er hendes tre situationer + Skriv selv. Én målgruppe → målgruppeskærmen springes over. Computeren (1440 i selen): hjulet 236 px midt i, svarkort 1032 px brede, knapperne i højre side. Designlåsen: 19 døre + 15 sider OK (kørt fra browserpanen, fordi headless Chrome på Macen crashede før målingen; koer.sh-fejlen var Chrome, ikke appen).
- **Ikke målt:** flere målgrupper (ingen kunde har to i basen); rigtig telefon. Kundens egne situationer bærer »hun/hende« (persona-data i VIRKSOMHEDSKODER, kræver Idas ord, jf. v1852). `storStart` løfter ikke »é« (»én detalje …« står med lille) — husets funktion, ikke rørt.

## 5bd. v1930: Drejebogen som værktøj i Idébanken (Idas klik 14/9 kl. 22.50)

`kundeArkAabn` › `ekstraAf("ide")`: er `KUNDE_ARK_SOEJLE` sat, tilføjes `<button class="ark-knap" onclick="kundeArkDrejebog()">Find idéen med Drejebogen</button>` under søjle-chipsene; `kundeArkSoejleVaelg` tegner `#arkEkstra` igen, så knappen følger valget. `kundeArkDrejebog()`: koden fra `soejleModel(navn)` (eller chippen selv, når kunden ingen Drejebog har og chipsene er koderne), lukker arket stille, nulstiller søjle/vedhæft, kalder `drejebogSpoergAabn(kode, navn, tekst)`. Den sætter `PR = { solo:true, trin:2, t2:{ skaerm:"spm"|"mg", forslag:[ét opslag uden dag], skriv: teksten } }` og bruger #planRit-skallen med klassen `pr-solo` (skinnen skjult, én kolonne max 760 px). I solo: `prBjaelke` tom, toplabel »Drejebogen«, `prFrem` tom, `prEyeSted(f)` skriver søjlenavnet i stedet for dagen, `prIdeHtml` uden dag-kort og med »Læg i Idébanken«, `prIdeGem` gemmer status Idé/dato null, lukker og tegner Idébanken; `planRitTilbage` fra første spørgsmål (eller målgruppeskærmen) lukker.

- **Målt i selen (390, v1930):** arket viser knappen (318 × 48) først efter et søjlevalg; tryk → fuld skærm »1A · Produkt · 1 af 3« med det skrevne i Skriv selv-feltet; tre skærme → »Her er idéen« med Titel + Formatet; »Læg i Idébanken« → IDEER får {titel: det skrevne, status Idé, dato null, soejle Produkt, brief.beskrivelse = spørgsmål+svar}, #planRit lukket. Tilbage fra første spørgsmål lukker. Designlåsen (browserpanen): 19 døre + 15 sider OK.
- **Ikke målt:** live hos kunden (kommer efter deploy), computeren, rigtig telefon.
