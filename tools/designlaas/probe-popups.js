  ...
  try {
    ...
    for (var i = 0; i < DOERE.length; i++) {
      ...
      if (rod) {
        ...
        var trykOk = function (b, r) { if (r.height >= 44) return true; try { var a = getComputedStyle(b, "::after"); if (a && a.content && a.content !== "none" && a.position === "absolute") { var ekstra = Math.max(0, -(parseFloat(a.top) || 0)) + Math.max(0, -(parseFloat(a.bottom) || 0)) - 2; if (r.height + ekstra >= 43.5) return true; } } catch (e) {} try { var cx = r.left + r.width / 2, cy = r.top + r.height / 2; var op = document.elementFromPoint(cx, cy - 21), ned = document.elementFromPoint(cx, cy + 21); return !!(op && b.contains(op) && ned && b.contains(ned)); } catch (e) { return false; } };
        knapper.forEach(function (b) { var r = b.getBoundingClientRect(); if (!trykOk(b, r)) { console.log("SELE LAAS FEJL " + navn + " knap under 44 px: " + b.textContent.trim().slice(0, 30) + " (" + Math.round(r.height) + ")"); fejl++; } if (r.width < rb.width * 0.85 && !b.closest(".pr-chips") && !b.classList.contains("kort-plus") && !/^(Slet|Tilføj til kalender|Fjern)$/.test(b.textContent.trim()))   /* 15/9: tilfoej-knappen er 44 px i EGEN bredde, ogsaa paa telefonen (Idas klik kl. 16.55) */ { console.log("SELE LAAS FEJL " + navn + " knap ikke lang: " + b.textContent.trim().slice(0, 30) + " (" + Math.round(r.width) + " af " + Math.round(rb.width) + ")"); fejl++; } });
      }
