...
setTimeout(async function () {
  try {
    ...
    for (var i = 0; i < SIDER.length; i++) {
      ...
      /* ALDRIG EN LISTE I EEN FLISE (HAARD, Ida 21/9 kl. 16.55): tre eller flere raekker med indbyrdes streger i samme flise = fejl */
      try { var lister = [].filter.call(r.querySelectorAll(".ws-card, .bs3-fl, .bs-kf"), function (k) { if (!k.getClientRects().length) return false; var rk = [].filter.call(k.children, function (c) { var cs = getComputedStyle(c); return c.getClientRects().length && (parseFloat(cs.borderTopWidth) > 0 || parseFloat(cs.borderBottomWidth) > 0) && !c.classList.contains("bs3-et") && !c.classList.contains("ws-head"); }); return rk.length >= 3; }); if (lister.length) { console.log("SELE BACKSTAGE FEJL " + navn + " liste i een flise (" + lister.length + "): " + (lister[0].className || lister[0].id)); fejl++; } } catch (e) {}
